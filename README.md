...
2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application? Redux as a whole is a state management system that helps track state actions and reducers just allow for changes in the state and the redux store holds all state including the default

3. What is the difference between Application state and Component state? When would be a good time to use one over the other? Application state is good id there is not much data to handle where as component state is good for bigger apps

4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`? Redux thunk is a middle ware it acts as the dispatcher for action creators
5. What is your favorite state management system you've learned and this sprint? Please explain why! Since more time was spent on redux id say redux why because its awesome !!! it handles alot of work that without would be a lot more to learn.
